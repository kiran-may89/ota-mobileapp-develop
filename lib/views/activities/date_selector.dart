...
class ActivityDateSelector extends StatefulWidget {
  Activity_Model model;
  BuildContext context;

  ActivityDateSelector(this.model, this.context);

  @override
  _ActivityDateSelectorState createState() => _ActivityDateSelectorState(this.model, this.context);
}

class _ActivityDateSelectorState extends State<ActivityDateSelector> {
  Activity_Model model;
  BuildContext context;

  _ActivityDateSelectorState(this.model, this.context);
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider.value(
        value: model,
        child: Consumer<Activity_Model>(builder: (context, model, child) {
          return
            Container(
...
//                  ),
  ...
  Widget _calendarBody() {
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        } else if (_selectedToDateTime != null &&
  ...
  Widget _calendarDates(Calendar calendarDate) {
    return InkWell(
      onTap: () {
        if (calendarDate.date.isAfter(DateTime.now()) || calendarDate.date == DateTime.now()) if (_selectedFromDateTime == null) {
          ...
          if(calendarDate.date.isAfter(_selectedFromDateTime))
          if (_selectedToDateTime != calendarDate.date) {
            if (calendarDate.nextMonth) {
              _getNextMonth();
            } else if (calendarDate.prevMonth) {
              _getPrevMonth();
            }
            setState(() => _selectedToDateTime = calendarDate.date);
            model.upReturnDate(calendarDate.date);
          }
          print("ToDateSelected");
        } else if (_selectedToDateTime != null && _selectedFromDateTime != null) {
      ...
      child: Container(
          ...
          child: Text('${calendarDate.date.day}',
              style: (calendarDate.date.isAfter(DateTime.now()) || calendarDate.date == DateTime.now())
...
//          ),
  ...
  void initState() {
    ...
    if(model.startDate!=null){
      DateTime dateTime = DateFormat("yyyy-MM-dd").parse(model.startDate);
    ...
    if(model.returnDate!=null){
      DateTime dateTime = DateFormat("yyyy-MM-dd").parse(model.returnDate);
  ...
  Widget _calendarDatesOneWay(Calendar calendarDate) {
    return InkWell(
      onTap: () {
        if (calendarDate.date.isAfter(DateTime.now()) || calendarDate.date == DateTime.now()) if (_selectedFromDateTime != calendarDate.date) {
          if (calendarDate.nextMonth) {
            _getNextMonth();
          } else if (calendarDate.prevMonth) {
            _getPrevMonth();
