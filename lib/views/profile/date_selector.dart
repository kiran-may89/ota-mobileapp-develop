...
class ProfileDateSelector extends StatefulWidget {

  ProfileViewModel model;
  BuildContext context;


  ProfileDateSelector(this.model, this.context);

  @override
  _ProfileDateSelectorState createState() => _ProfileDateSelectorState(this.model, this.context);
}

class _ProfileDateSelectorState extends State<ProfileDateSelector> {

  ProfileViewModel model;
  BuildContext context;

  _ProfileDateSelectorState(this.model, this.context);
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index){
        if(index < 7) return _weekDayTitle(index);
          ...
          if(_sequentialDates[index - 7].date == _selectedDOB) {
...
//  }
  ...
  void initState() {
    ...
    if(model.dob!=null){
      ...
      _currentDateTime = DateTime(model.dob.year, model.dob.month);
  ...
  Widget _calendarDatesOneWay(Calendar calendarDate) {
    ...
    return InkWell(
      onTap: (){
        if(calendarDate.date.isBefore(DateTime.now())||calendarDate.date==DateTime.now())


          if (_selectedDOB != calendarDate.date) {
            if (calendarDate.nextMonth) {
              _getNextMonth();
            }
            else if (calendarDate.prevMonth) {
            ...
            model.setTempDOB(calendarDate.date);

            setState(() => _selectedDOB = calendarDate.date);
          }
      ...
      child: Container(
          ...
          child: Text(
              '${calendarDate.date.day}',
              style:
              (calendarDate.date.isBefore(DateTime.now())||calendarDate.date==DateTime.now())?
              CustomStyles.calenderStyle.copyWith(color: CustomColors.heading):CustomStyles.calenderStyle
